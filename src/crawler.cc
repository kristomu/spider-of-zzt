...
// Reads initial URLs from file and doesn't crawl. TEST -km
...
int main(void)
{
  ...
  std::vector<easy_handle_container> c;

  while (std::getline(URL_list_file, line)) {
    easy_handle_container * cur = new easy_handle_container(line);
    curl_multi_add_handle(multi_handle, cur->handle);
    //curl_multi_add_handle(multi_handle, make_handle(line.data()));
  }
  ...
  while(still_running && !pending_interrupt) {
    ...
    printf("numfds: %d still running: %d\n", numfds, still_running);
      ...
      if(m->msg == CURLMSG_DONE) {
        ...
        if(m->data.result == CURLE_OK) {
          ...
          if(res_status == 200) {
            ...
            if(is_html(ctype) && mem->data_buffer.size() > 100) {
